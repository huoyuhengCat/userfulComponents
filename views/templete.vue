<template>
  <div>
    ...
    <Dialog title="详情" :visible.sync="modalDialogVis" save :confirm="modalConfirm" :open="modalOpen" :sureText="mode==='detail'?'确认':'保存'">
      <Forms formName="modalForm" :config="myForm" ref="modalForm"></Forms>
    </Dialog>
  </div>
</template>
<script>
...
export default {
  ...
  data () {
    return {
      ...
      myForm: {
        'code': { name: '接口编号', required: true },
        'name': { name: '接口名称', required: true },
        'createBy': { name: '创建人', required: false },
        'url': { name: '访问路径', required: true },
        'version': { name: '版本', required: true },
        'isDefault': { name: '是否默认', type: 'select', data: [{ value: true, label: '是' }, { value: false, label: '否' }], default: true, required: true },
