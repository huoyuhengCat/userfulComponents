...
import { orgRoles, listAllRoles } from '@/api/role'
import { userDetail, saveUser, editUser, getUserList, deleteUser } from '@/api/user'
...
export default {
  ...
  methods: {
    async businessFetch (queryData) {
      let res = await getUserList(queryData).then(res => res).catch(() => false)
      return res
    },
    addeditHandler (val) {
      ...
      if (val !== 'add') {
        ...
        this.$nextTick(async () => {
          ...
          if (!res) { return false }
          let orgRes = await orgRoles({ orgId: res.orgId }).then(res => res).catch(() => false)
          this.options = orgRes
          console.log('编辑信息', orgRes)
